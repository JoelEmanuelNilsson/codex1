...
pub fn run(args: UninstallArgs) -> Result<()> {
    ...
    for entry in &manifest.paths {
        if !entry.applied {
            planned.push(PlannedUninstallAction::Noop {
                path: resolve_manifest_target_path(&repo_root, &user_root, entry)?,
    ...
    for (index, action) in planned.into_iter().enumerate() {
        ...
        match action {
            PlannedUninstallAction::WriteFile {
                ...
                transaction_mutations.push(SupportSurfaceMutation {
                    ...
                    kind: SupportSurfaceMutationKind::WriteFile { contents },
                    success_label: label.to_string(),
            ...
            PlannedUninstallAction::DeleteFile { path, label } => {
                transaction_mutations.push(SupportSurfaceMutation {
                    ...
                    success_label: label.to_string(),
                    failure_label: "failed_delete_file".to_string(),
                    missing_label: Some(label.to_string()),
                });
            }
            PlannedUninstallAction::Noop { path, label } => {
                transaction_mutations.push(SupportSurfaceMutation {
                    path,
                    manifest_index: Some(index),
                    kind: SupportSurfaceMutationKind::Noop,
                    success_label: label.to_string(),
                    failure_label: label.to_string(),
                    missing_label: None,
                });
            }
        }
    ...
    .context("execute support-surface uninstall transaction")?;
    removed_paths.extend(transaction.outcomes.into_iter().map(|outcome| PathOutcome {
        path: outcome.path,
